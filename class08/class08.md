...
In today’s class we will applu the methods and techniques clustering and
PCA to help make sense of a real world breast cancer FNA biopsy data
set.

## Data Import

We start by importing out data. It is a csv file so we will use the
`read.csv()` function
...
Three principal components are required to describe 72.64% of the
original variance.

> Q6. How many principal components (PCs) are required to describe at
> least 90% of the original variance in the data?

Seven principal components are required to describe 91.01% of the
original variance.
...
There’s too much information on the plot making it very difficult to
understand because the labels and points are all over the place.
...
These plots are much more organized and easier to understand, allowing
us to differentiate between malignant and benign.
...
Collectively these two plot (“score plot” and “loadings plot”) tell us
that if cells nucleus are deeply indented (“concave”), irregular non
...
The loading of concave.points_mean on PC1 is −0.2609, indicating a
strong contribution to the first PC. Yes, there are several features
have larger contributions, including concavity_mean,
concave.points_worst, and size-related features such as perimeter_worst
and radius_worst.
...
I like the ward.D2 method because it creates groups such that variance
is minimized within clusters.
...
The newly created hclust model separates out the two “M” and “B”
diagnoses really well because there are now two distinct big groups in
...
The hierarchical clustering done without PCA somewhat separates benign
and malignant case, but there is still a lot of overlap between the
clusters. When clustering is done after PCA, the separation is much
clearer, with one cluster mostly malignant and the other mostly benign.
Overall, using PCA before hierarchical clustering gives better
separation of the diagnoses.
...
We should prioritize patient 2 because they are likely be be diagnosed
as malignant compared to patient 1. This is because patient 2 falls
inside the group with the most malignant cases.
